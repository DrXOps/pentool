# ⚡ Pentool — AI-пентест прямо в терминале

> **Быстро, легко, без тормозов.** AI-ассистент ведёт пентест в терминале — подбирает
> правильные чеки, обходит WAF, находит скрытые эндпоинты. Без тяжёлых IDE и лагов.
...
Объединяет перехват трафика, сканирование уязвимостей, AI-помощника, автоматизированные атаки и анализ данных — всё в одном терминале.  
Быстро, удобно, прозрачно.

**Ваш AI на подхвате:** укажите URL один раз — Pentool включит прокси, сгенерирует
и импортирует доверенный CA-сертификат в headless-браузер, отправит первый запрос и
наполнит проект — можно сразу начинать аудит. Быстро, без лишней настройки.

**CI/CD без TUI:** `pentool --url https://example.com --headless --output result.json` —
